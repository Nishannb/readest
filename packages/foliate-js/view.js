...
export class View extends HTMLElement {
    ...
    #handleLinks(doc, index) {
        ...
        doc.addEventListener('click', e => {
            ...
            if (book?.isExternal?.(href))
                ...
                const canResolve = !!book.resolveHref(href)
                if (!canResolve) {
                ...
                Promise.resolve(this.#emit('link', { a, href: internalHref }, true))
                    .then(x => {
                        if (!x) return null
                        const resolved = book.resolveHref(internalHref)
                        if (!resolved) {
                            try { window.dispatchEvent(new CustomEvent('foliate-go-to-failed', { detail: { stage: 'link-precheck', target: internalHref } })) } catch {}
                            return null
                        }
                        return this.goTo(internalHref)
                    })
                    .catch(e => {
                        try { window.dispatchEvent(new CustomEvent('foliate-go-to-failed', { detail: { stage: 'link', target: internalHref, error: e } })) } catch {}
                    })
            }
        })
    }
    async addAnnotation(annotation, remove) {
        ...
        if (this.book.type === 'pdf' && value.startsWith('pdf_annotation_')) {
            if (remove) {
                // Remove PDF annotation
                if (this.book.removePDFAnnotation) {
                    const annotationId = value.replace('pdf_annotation_', '')
                    this.book.removePDFAnnotation(annotationId)
                }
    ...
    getCFI(index, range) {
        // For PDFs, create a custom identifier since CFI doesn't work
        if (this.book.type === 'pdf') {
            if (!range) return `pdf_page_${index}`
            // Create a PDF-specific identifier using page and coordinates
            const rects = Array.from(range.getClientRects())
            if (rects.length > 0) {
                const rect = rects[0]
                return `pdf_page_${index}_${Math.round(rect.left)}_${Math.round(rect.top)}_${Math.round(rect.width)}_${Math.round(rect.height)}`
            }
            return `pdf_page_${index}`
        }
    ...
    async goTo(target) {
        ...
        try {
            ...
            try { window.dispatchEvent(new CustomEvent('foliate-go-to-failed', { detail: { stage: 'goto', target, error: e } })) } catch {}
        }
    }
    ...
    async select(target) {
        try {
            ...
            try { window.dispatchEvent(new CustomEvent('foliate-go-to-failed', { detail: { stage: 'select', target, error: e } })) } catch {}
        }
    }
    deselect() {
