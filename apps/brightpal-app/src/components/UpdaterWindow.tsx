...
import { BRIGHTPAL_UPDATER_FILE, BRIGHTPAL_CHANGELOG_FILE } from '@/services/constants';
...
export const UpdaterContent = ({
  ...
  useEffect(() => {
    ...
    const checkAndroidUpdate = async () => {
      ...
      const fetch = isTauriAppPlatform() ? tauriFetch : window.fetch;
              const response = await fetch(BRIGHTPAL_UPDATER_FILE);
  ...
  useEffect(() => {
    const fetchChangelogs = async (fromVersion: string): Promise<Changelog[]> => {
      try {
        const fetch = isTauriAppPlatform() ? tauriFetch : window.fetch;
        const res = await fetch(BRIGHTPAL_CHANGELOG_FILE);
      ...
      <div className='flex w-full max-w-2xl flex-col gap-4'>
        <div className='flex flex-col justify-center gap-4 sm:flex-row sm:items-start'>
          <div className='flex items-center justify-center'>
            <Image src='/favicon.ico' alt='Logo' className='h-20 w-20' width={64} height={64} />
          </div>
                ...
                {changelogs.length > 0 && semver.gt(changelogs[0]!.version, currentVersion) ? (
                  <div className='flex gap-2'>
                    ...
                    {appService?.isAndroidApp && (
                      <Link
                        href='https://play.google.com/store/apps/details?id=com.nishan.brightpal'
                        target='_blank'
                        rel='noopener noreferrer'
                        className='btn btn-primary btn-sm'
                      >
