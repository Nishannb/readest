...
type AIChatState = {
  ...
  abortControllerByBookKey: PerBookMap<AbortController | null>;
  ...
  stopStreaming: (bookKey: string) => void;
  setAbortController: (bookKey: string, controller: AbortController | null) => void;
  getAbortController: (bookKey: string) => AbortController | null;
...
export const useAIChatStore = create<AIChatState>()(
  ...
  clear: (bookKey) =>
    set((s) => {
      ...
      return {
        ...
        abortControllerByBookKey: { ...s.abortControllerByBookKey, [bookKey]: null },
      };
    }),
  ...
  stopStreaming: (bookKey) => {
    const s = get();
    const controller = s.abortControllerByBookKey[bookKey];
    if (controller) {
      controller.abort();
    }
    set((s) => ({
  ...
  setAbortController: (bookKey, controller) =>
    set((s) => ({ 
      abortControllerByBookKey: { ...s.abortControllerByBookKey, [bookKey]: controller } 
    })),

  getAbortController: (bookKey) => get().abortControllerByBookKey[bookKey],
