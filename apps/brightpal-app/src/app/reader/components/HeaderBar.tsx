...
import { startMacScreenCapture } from '@/utils/screenshot';
...
const HeaderBar: React.FC<HeaderBarProps> = ({
  ...
  // Get book format to conditionally show shortcuts dropdown
  const bookData = useBookDataStore((s) => s.getBookData(bookKey));
  const isPDF = bookData?.book?.format === 'PDF';
  const isHeaderVisible = true; // This should be derived from the actual logic
  ...
  const handleTakeScreenshot = useCallback(async () => {
    try {
      if (appService?.appPlatform === 'macos') {
        const controller = await startMacScreenCapture((result) => {
          if (result) {
            // Add screenshot to AI context
            addAIContext(bookKey, result.imageUrl);
            // Show AI panel
        ...
        // For non-macOS platforms, we could implement a different screenshot method
        console.log('Screenshot not supported on this platform');
      }
    } catch (error) {
      console.error('Screenshot failed:', error);
    }
  }, [appService?.appPlatform, addAIContext, bookKey, setAIPanelVisible]);

  // Handle keyboard shortcuts
  useShortcuts({
    onToggleAIChat: () => toggleAIVisible(bookKey),
    onTakeScreenshot: handleTakeScreenshot,
  }, [bookKey, toggleAIVisible, handleTakeScreenshot]);
