...
import { FiSend, FiX, FiSettings, FiPlus, FiRefreshCw, FiSquare } from 'react-icons/fi';
...
const InputRow: React.FC<{ bookKey: string; value: string; onChange: (v: string)=>void; onSend: ()=>void; onStop: ()=>void }> = ({ bookKey, value, onChange, onSend, onStop }) => {
  const isStreaming = useAIChatStore((s) => s.isStreamingByBookKey[bookKey]);
  ...
  const selectedModel = (() => {
    switch (selectedProvider) {
      ...
      case 'openrouter':
        return provider.openrouter.model || 'meta-llama/llama-4-maverick:free';
      case 'ollama':
        return provider.ollama.model || 'llama3.2';
      case 'selfHosted':
        return provider.selfHosted.model || 'mistral:7b';
      default:
        return 'model';
    }
  })();
  ...
  return (
    <div className='mb-0 flex items-end gap-2 border border-base-300 bg-base-200/20 px-3 py-2 shadow-sm w-full'>
      <textarea
        ...
        placeholder={`Ask anything to selected ${getProviderDisplayName(selectedProvider)} model`}
        value={value}
        rows={4}
...
const OpenRouterModelPicker: React.FC = () => {
  ...
  const load = React.useCallback(async () => {
    ...
    try {
      ...
      // Get all models and filter for free ones, prioritizing vision models
      const allModels = Array.isArray(data?.data) ? data.data : [];
      const freeModels = allModels
        .filter((m: any) => typeof m?.id === 'string' && /:free$/.test(m.id))
        .map((m: any) => m.id as string);
      
      // Check for vision-capable models
      const visionModels = freeModels.filter(id => 
        id.toLowerCase().includes('vision') || 
        id.toLowerCase().includes('llama-3.2-11b-vision') ||
        id.toLowerCase().includes('claude-3.5-sonnet') ||
        id.toLowerCase().includes('gemini-pro-vision')
      );
      
      // Sort vision models alphabetically, then other free models alphabetically
      const sortedVisionModels = visionModels.sort((a, b) => a.localeCompare(b));
      const otherModels = freeModels.filter(id => !visionModels.includes(id)).sort((a, b) => a.localeCompare(b));
      const sortedModels = [...sortedVisionModels, ...otherModels];
      
      setModels(sortedModels);
      if (sortedModels.length > 0) {
        const cur = provider.openrouter.model || '';
        if (!cur || !sortedModels.includes(cur)) {
          // Prefer a vision model if available
          const preferredModel = visionModels.length > 0 ? visionModels[0] : sortedModels[0];
          provider.saveOpenRouter({ model: preferredModel });
        }
      }
    } catch (e) {
      setError((e as Error)?.message || 'Failed to fetch models');
  ...
  const isVisionModel = (modelId: string) => {
    return modelId.toLowerCase().includes('vision') || 
           modelId.toLowerCase().includes('llama-3.2-11b-vision') ||
  ...
  return (
    <div className='grid gap-2 text-xs'>
      ...
      {provider.openrouter.model && isVisionModel(provider.openrouter.model) && (
        <div className='text-[11px] text-success'>
          ✅ This model supports images and text (multimodal)
        </div>
      )}
      {error && <div className='text-[11px] text-error'>{error}</div>}
    </div>
...
const AIChatPanel: React.FC<{ bookKey: string }> = ({ bookKey }) => {
  ...
  const sendPrompt = useCallback(async () => {
    ...
    const setAbortController = useAIChatStore.getState().setAbortController;
    setAbortController(bookKey, abortController);

    // Unified multimodal context processing
    const processMultimodalContext = async (context: string[] | undefined) => {
      ...
      for (const t of ctx) {
        ...
        if (dataMatch) {
          ...
          if (mimeType && base64Data && base64Data.length > 0) {
            ...
            console.log('Added data URI image:', { mimeType, dataLength: base64Data.length });
          } else {
            console.warn('Invalid data URI format:', t);
          }
          continue;
        }
        
        // Check if it's a markdown image or direct URL
        let src = '';
        const md = t.match(/!\[[^\]]*\]\(([^)]+)\)/);
        if (md) src = md[1] || '';
        ...
        if (src) {
          if (/^data:image\//i.test(src)) {
            ...
            // Keep as direct URL for better provider compatibility
            imageParts.push({
              type: 'image_url',
              image_url: { url: src }
            });
          } else {
            // Unsupported scheme, treat as text
            textParts.push(t);
          }
        } else {
    ...
    try {
      ...
      if (provider.defaultProvider === 'gemini') {
        ...
        const isVisionModel = modelName.toLowerCase().includes('vision') || 
                             modelName.toLowerCase().includes('llama-3.2-11b-vision') ||
        ...
        console.log('OpenRouter model detection:', {
          modelName,
          isVisionModel,
          lowercaseModel: modelName.toLowerCase()
        });
        
        // Process multimodal context for OpenRouter
        const { imageParts, textParts } = await processMultimodalContext(boundContext);
        
        // Debug logging for multimodal context processing
        console.log('Multimodal context processing:', {
          boundContext,
          imageParts,
          textParts,
        ...
        let messagesForOR: any[] = [
          ...(systemPreamble ? [{ role: 'system', content: systemPreamble }] : []),
        ];
        
        if (isVisionModel && imageParts.length > 0) {
          // For vision models, send text and images in a single message with content array
          ...
          // For non-vision models, send text-only
          const userContent = [
        ...
        if (!res.ok) {
          ...
          if (res.status === 429) {
            console.log('Rate limited by OpenRouter, waiting before retry...');
            // Wait 2 seconds before retry
            await new Promise(resolve => setTimeout(resolve, 2000));
            
            console.log('Retrying OpenRouter request after rate limit...');
            ...
            if (retryRes.ok) {
              ...
              console.error('OpenRouter retry also failed:', retryErrorText);
              
              // Show user-friendly rate limit message in chat UI
              const rateLimitMsg = '⚠️ You have crossed Open Router limit, please try again later or change provider';
              appendAssistantChunk(bookKey, rateLimitMsg);
              return;
            }
          }
          
          // If it's a vision model and we got an error, try a fallback approach
          if (isVisionModel && imageParts.length > 0) {
            console.log('Trying fallback approach for OpenRouter vision model...');
            
            // Fallback: try sending as a single message with simpler structure
            ...
            console.log('OpenRouter fallback request:', {
              model: modelName,
            ...
            if (fallbackRes.ok) {
              ...
              console.error('OpenRouter fallback also failed:', fallbackErrorText);
              ...
              if (fallbackRes.status === 400) {
                try {
                  ...
                  const providerName = errorData?.error?.metadata?.provider_name || 'Unknown';
                  const rawError = errorData?.error?.metadata?.raw;
                  
                  let userMessage = '';
                  if (rawError && rawError.includes('Input validation error')) {
                    userMessage = `❌ Something went wrong. Please try a different model or provider. You could also try after some time with same model.`;
                  } else {
                    userMessage = `❌ Invalid request to ${providerName}. Please check your input and try again.`;
                  }
                  
                  appendAssistantChunk(bookKey, userMessage);
                  return;
                } catch (parseError) {
                  appendAssistantChunk(bookKey, '❌ Invalid request format. Please check your input and try again.');
                  return;
                }
              }
              ...
              if (fallbackRes.ok) {
                ...
                if (fallbackRes.status === 400) {
                  ...
                  appendAssistantChunk(bookKey, '❌ Invalid API key. Please check your OpenRouter API key and try again.');
                  return;
                } else if (fallbackRes.status === 403) {
                  appendAssistantChunk(bookKey, '❌ Access denied. Please check your OpenRouter account permissions.');
                  return;
                } else if (fallbackRes.status >= 500) {
                  appendAssistantChunk(bookKey, '❌ OpenRouter service is temporarily unavailable. Please try again later.');
                  return;
                }
                
                // If we can't parse the error, show a generic message
                appendAssistantChunk(bookKey, '❌ OpenRouter request failed. Please try again or use a different model.');
                return;
              }
            }
          }
          
          // Show error message in chat UI instead of throwing
          const errorMsg = '❌ OpenRouter request failed. Please check your API key or try a different model.';
          appendAssistantChunk(bookKey, errorMsg);
          return;
        }
        const data = await res.json();
      ...
      const setAbortController = useAIChatStore.getState().setAbortController;
      setAbortController(bookKey, null);
    }
    ...
    try {
      ...
      const [newSummary, rawNotes] = await Promise.all([
        genOnce(SUMMARY_PROMPT).catch((e) => {
          if (e instanceof Error && e.name === 'AbortError') return '';
          return '';
        }),
        genOnce(EXTRACT_PROMPT).catch((e) => {
          if (e instanceof Error && e.name === 'AbortError') return '';
          return '';
        }),
      ]);
    ...
    >
      ...
      <div className='border-t border-base-300 px-2 pt-2 pb-0'>
        ...
        <InputRow
          ...
          onSend={sendPrompt}
          onStop={() => {
            const stopStreaming = useAIChatStore.getState().stopStreaming;
            stopStreaming(bookKey);
          }}
        />
      </div>
    </div>
