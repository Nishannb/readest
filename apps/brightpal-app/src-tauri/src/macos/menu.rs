...
pub fn setup_macos_menu(app: &AppHandle) -> tauri::Result<()> {
    ...
    global_menu.append(
        &SubmenuBuilder::new(app, "Help")
            ...
            .text("brightpal_help", "BrightPal Help")
            .build()?,
    )?;
...
pub fn handle_menu_event(app: &AppHandle, event: &MenuEvent) {
    ...
    if event.id() == "privacy_policy" {
        let _ = opener.open_url("https://brightpal.com/privacy-policy", None::<&str>);
    } else if event.id() == "report_issue" {
        let _ = opener.open_url("https://github.com/Nishannb/readest/issues", None::<&str>);
    } else if event.id() == "brightpal_help" {
        let _ = opener.open_url("https://brightpal.com/support", None::<&str>);
    }
}
